...
interface SummaryBase {
  transactionCount: number;
  totalGross: number;
  totalSchoolRecieves: number;
  totalFees: number;
  totalDonorCovers: number;
  byMethod: Partial<Record<PaymentMethod, MethodSummary>>;
}
...
export class FeeCalculator {
  transactionsByCompaign: Record<string, TransactionRecord[]>;
  currentTransactionNum: number;

  static readonly PLATFORM_RATE = 0.035;
  static readonly CARD_PCT_RATE = 0.029;
  static readonly CARD_FIXED_FEE = 0.3;
  static readonly ACH_PCT_RATE = 0.008;
  static readonly ACH_MAX_FEE = 5.0;

  constructor() {
    this.transactionsByCompaign = {};
  ...
  calculateFees(gross: number, method: PaymentMethod): FeeBreakdown {
    if (gross <= 0) {
      throw new Error("gross must be more than 0");
    }
    const platformFee = Math.round(gross * 0.035 * 100) / 100;
    let processingFee = 0;
    if (method === "card") {
      processingFee = Math.round((gross * 0.029 + 0.3) * 100) / 100;
    } else if (method === "ach") {
      processingFee = Math.min(Math.round(gross * 0.008 * 100) / 100, 5);
    }
    ...
    return {
      ...
      processingFee,
      totalFees,
      schoolReceives: gross - totalFees,
    };
  }
  ...
  grossForNet(netTarget: number, method: PaymentMethod): FeeBreakdown {
    ...
    // schoolReceives = gross - processingFees - platformFees
    let minGross = 0;
    if (method == "card") {
      // schoolReceives = gross - (gross*.029 + .3) - (gross * .035)
      // schoolReceives = gross - .064*gross -.3
      //                = .936*gross -.3
      minGross = (netTarget + 0.3) / 0.936;
    } else if (method == "ach") {
      // case 1: netTargt/.957 = gross
      // cass 2 : (netTarget + 5)/.965 = gross
      minGross = Math.min(netTarget / 0.957, (netTarget + 5) / 0.965);
    } else {
      minGross = netTarget / 0.965;
    }
    return this.calculateFees(Math.round(minGross * 100) / 100, method);
  }
  ...
  recordTransaction(
    ...
    let transactionFees = this.calculateFees(gross, method);
    // if (donorCoversFee) {
    //   transactionFees = this.grossForNet(gross, method);
    // }
    this.currentTransactionNum += 1;
    const newTransactionRecord: TransactionRecord = {
      id: `tx-${this.currentTransactionNum}`,
      campaignId,
      gross: transactionFees.gross,
      schoolReceives: transactionFees.schoolReceives,
      totalFees: transactionFees.totalFees,
      method,
      donorCoversFee,
    };
    const existingTransactionsByCampaign =
      this.transactionsByCompaign[campaignId];
    if (existingTransactionsByCampaign) {
      existingTransactionsByCampaign.push(newTransactionRecord);
    } else {
      this.transactionsByCompaign[campaignId] = [newTransactionRecord];
    }
  ...
  getCampaignFeeSummary(campaignId: string): CampaignFeeSummary {
    const campaignTransactions = this.transactionsByCompaign[campaignId];
    if (!campaignTransactions) {
      throw new Error("no campaign transactions");
    }

    const summaryBase = campaignTransactions.reduce(
      (summary: SummaryBase, transaction) => {
        const donorIncrease = transaction.donorCoversFee ? 1 : 0;
        let newByMethod: MethodSummary = {
          count: 1,
          totalGross: transaction.gross,
          totalFees: transaction.totalFees,
          totalSchoolReceives: transaction.schoolReceives,
        };
        const existingByMethod = summary.byMethod[transaction.method];
        if (existingByMethod) {
          newByMethod = {
            count: existingByMethod.count + 1,
            totalGross: existingByMethod.totalGross + transaction.gross,
            totalFees: existingByMethod.totalFees + transaction.totalFees,
            totalSchoolReceives:
              existingByMethod.totalSchoolReceives + transaction.schoolReceives,
          };
        }
        const newByMethodRecord = {
          ...summary.byMethod,
          [transaction.method]: newByMethod,
        };
        return {
          transactionCount: summary.transactionCount + 1,
          totalGross: summary.totalGross + transaction.gross,
          totalSchoolRecieves:
            summary.totalSchoolRecieves + transaction.schoolReceives,
          totalFees: summary.totalFees + transaction.totalFees,
          totalDonorCovers: summary.totalDonorCovers + donorIncrease,
          byMethod: newByMethodRecord,
        };
      },
      {
        transactionCount: 0,
        totalGross: 0,
        totalSchoolRecieves: 0,
        totalFees: 0,
        totalDonorCovers: 0,
        byMethod: {},
      },
    );
    const feeCoverageRate =
      Math.round(
        (summaryBase.totalDonorCovers / summaryBase.transactionCount) * 100,
      ) / 100;

    return {
      campaignId,
      transactionCount: summaryBase.transactionCount,
      totalGross: summaryBase.totalGross,
      totalSchoolReceives: summaryBase.totalSchoolRecieves,
      totalFees: summaryBase.totalFees,
      feeCoverageRate,
      byMethod: summaryBase.byMethod,
    };
  }
}
